...
export default function TestimonialsAndStandoutSection() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [testRes, standRes] = await Promise.all([
          axios.get(`${cleanBaseUrl}/api/home/testimonials`),
          axios.get(`${cleanBaseUrl}/api/home/standout`)
        ]);
        setTestimonialData(testRes.data);
        setStandOutData(standRes.data);
      } catch (error) {
      ...
      <div className="container">
        <div className="row align-items-center g-5">
          ...
          <div className="col-lg-6">
            {testimonialData && (
              <>
                <h2 className="fw-bold mt-2 mb-3">{testimonialData.heading}</h2>
                <p className="text-muted mb-4">{testimonialData.description}</p>
                ...
                >
                  {testimonialData.items?.map((item, index) => (
                    <SwiperSlide key={index}>
                      <div className="testimonial-box position-relative bg-white p-4 shadow-sm rounded">
                        <p className="mt-2 italic">“{item.text}”</p>
                        <div className="d-flex align-items-center mt-4">
                          <img
                            src={`${cleanBaseUrl}${item.image}`}
                            alt={item.name}
                          ...
                          <div className="ms-3">
                            ...
                            <br />
                            <small>{item.role}</small>
                            <p className="testimonial-stars mb-0 text-warning">
                              {[...Array(Number(item.rating) || 5)].map((_, i) => (
                                <i key={i} className="fa fa-star pe-1"></i>
                              ))}
                            </p>
                          </div>
                        </div>
                        <span className="quote-mark" style={{ opacity: 0.1, position: 'absolute', right: '20px', bottom: '10px', fontSize: '3rem' }}>”</span>
                      </div>
                    </SwiperSlide>
                  ))}
                </Swiper>
          ...
          <div className="col-lg-6">
            {standOutData && (
              <>
                <h2 className="fw-bold mt-2 mb-3 text-center">{standOutData.heading}</h2>
                <div className="row g-4">
                  {standOutData.items?.map((item, index) => (
                    <div className="col-md-4" key={index}>
                      <div className="stat-box text-center p-3 h-100 border rounded bg-white shadow-sm">
                        <img 
