...
export default function Homas() {
  ...
  const doDelete = async () => {
    try {
      await api.delete(`/homas/${toDelete.id}`);
      setHomas((prev) => prev.filter((x) => x.id !== toDelete.id));
      setShowDelete(false);
      setToDelete(null);
    } catch (err) {
      console.error(err);
  ...
  return (
    <AdminLayout>
            ...
            <tbody>
              {current.map((h) => (
                <tr key={h.id}>
                  ...
                  <td className="mh-actions">
                    <button className="mh-icon-btn edit" onClick={() => openEdit(h)}>
                      <i className="fas fa-edit"></i>
                    </button>

                    <button
                      className="mh-icon-btn delete"
      ...
      <Modal
        ...
        backdrop="static"
        keyboard={false}
        container={document.body} //  MOST IMPORTANT FIX
      >
        <Modal.Header closeButton>
          <Modal.Title>Delete Homa</Modal.Title>
        </Modal.Header>
